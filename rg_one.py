...
and Random Matrix Theory. Integrates high‑precision zero computation
with differentiable particle simulations and statistical analysis.
...
class RHTrainer:
    ...
    def compute_loss(self, positions: torch.Tensor) -> torch.Tensor:
        ...
        return loss_s + 0.5 * loss_pc + 0.1 * loss_sff
...
def parse_args():
    ...
    p.add_argument('--mode', default='train', choices=['train','zeros','stats','demo'])
...
def main():
    ...
    if args.mode == 'zeros':
        ...
        s = unfold_torch(torch.tensor(zeros).unsqueeze(0)).squeeze().numpy()
        ...
        if HAS_MPL:
            ...
            plt.plot(s, (32/np.pi**2)*s**2*np.exp(-4*s**2/np.pi), 'r-', label='GUE')
            plt.legend()
